...
const faker = require("faker"); // this library is used to generate fake dataaa
  ...
  async function seedProducts(titlesArr, imgsArr, categStr) {
    try {
      ...
      for (let i = 0; i < titlesArr.length; i++) {
        let quantity = 25; // Default quantity
        
        // Set specific quantities for certain categories
        if (categStr === "Briefcases" && i === 0) {
          quantity = 0; // First briefcase has 0 quantity
        } else if (categStr === "Backpacks" && i < 4) {
          quantity = 0; // First 4 backpacks have 0 quantity
        }
